<template>
    <Teleport to="body" v-if="show && meetingToDelete">
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <p class="text-sm text-gray-900 dark:text-gray-100">
                                ...
                                <span class="font-semibold">{{ employeeName }}</span>'s meeting notice —
                                <span class="font-semibold">{{ meetingTimeDisplay }}: {{ meetingToDelete.particulars }}</span>?
                            </p>
...
const props = defineProps<{
    show: boolean;
    meetingToDelete: NoticeOfMeetingRecord | null;
    employeeName: string;
    deleting: boolean;
}>();
...
const meetingTimeDisplay = computed(() => {
    if (!props.meetingToDelete?.time) return '';
    const [hoursStr, minutesStr] = props.meetingToDelete.time.split(':');
